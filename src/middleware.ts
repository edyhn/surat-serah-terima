...
/**
 * ISSUE-30: Route yang dikecualikan dari autentikasi Supabase.
 * Route ini memiliki mekanisme autentikasi sendiri (cookie sesi signing / cleanup key).
 */
const SIGNING_PUBLIC_ROUTES = [
  ...
  /^\/api\/signing\/cleanup/,   // Cleanup job (validasi via internal key di route handler)
];

export async function middleware(request: NextRequest) {
  const pathname = request.nextUrl.pathname;

  // Lewati middleware Supabase untuk route signing publik/eksternal
  if (SIGNING_PUBLIC_ROUTES.some((pattern) => pattern.test(pathname))) {
    const response = NextResponse.next({ request });
    // Terapkan security headers dasar untuk semua route signing
