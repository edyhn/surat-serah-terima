...
export function Input({ label, error, helper, className, id, name, disabled, ...props }: InputProps) {
  ...
  return (
    <div className="space-y-1.5">
      ...
      <input
        ...
        aria-describedby={error && inputId ? `${inputId}-error` : helper ? helperId : undefined}
        className={cn(
      ...
      {error ? (
        ...
        <span id={helperId} className="block text-[11px] text-slate-500">
          {helper}
        </span>
      ) : null}
    </div>
