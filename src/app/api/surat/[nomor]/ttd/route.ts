...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    const input = ttdSchema.parse(await readJson(request));
    const signatures = parseTtd(input.ttd);
    if (input.nama && Object.keys(signatures).length === 1 && signatures.hrd) {
      surat.namaHrd = input.nama;
      await updateSurat(nomor, surat);
    }

    await saveTtd(nomor, signatures);
    ...
    return NextResponse.json({ ...surat, pdf: pdf.namaFile, ttd: ttdDataUrls(stored) });
  } catch (error) {
