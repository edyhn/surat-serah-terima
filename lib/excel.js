...
const KOLOM_DEF = [
  ...
  { header: 'Nama HRD', key: 'namaHrd', width: 25 },
  { header: 'Aset', key: 'aset', width: 45 },
];
...
function barisKeSurat(r) {
  return {
    ...
    namaHrd: r.getCell(11).value,
    aset: r.getCell(12).value,
  };
}
...
async function tambahRiwayat(s) {
  return seri(async () => {
    ...
    const aset = Array.isArray(s.aset) ? s.aset.join(', ') : (s.aset || '');
    ws.addRow([no, s.nomor, s.tanggal, s.tanggalSingkat, s.kategori, s.nama, s.departemen, s.penerima, s.departemenPenerima, s.keterangan, s.namaHrd || '', aset]);
    beriStyle(ws);
...
async function updateRiwayat(nomor, s) {
  return seri(async () => {
    ...
    row.getCell(11).value = s.namaHrd || '';
    row.getCell(12).value = Array.isArray(s.aset) ? s.aset.join(', ') : (s.aset || '');
    beriStyle(ws);
    await wb.xlsx.writeFile(FILE);
...
async function bikinExcel(daftar) {
  ...
  urut.forEach((s, i) => {
    const aset = Array.isArray(s.aset) ? s.aset.join(', ') : (s.aset || '');
    ws.addRow([i + 1, s.nomor, s.tanggal, s.tanggalSingkat, s.kategori, s.nama, s.departemen, s.penerima, s.departemenPenerima, s.keterangan, s.namaHrd || '', aset]);
  });
  beriStyle(ws);
