...
async function ambilPdf(namaFile) {
  const sb = pastikan();
  await pastikanBucket();
  const { data, error } = await sb.storage.from(BUCKET).download(namaFile);
  if (error) throw error;
...
async function muatTtd(nomor) {
  ...
  for (const pihak of PARTY_TTD) {
    const { data, error } = await sb.storage.from(BUCKET).download(namaTtd(nomor, pihak));
    if (data && !error) out[pihak] = Buffer.from(await data.arrayBuffer());
  }
  return out;
}

async function statusSemuaTtd() {
  const sb = pastikan();
  await pastikanBucket();
...
async function statusTtd(nomor) {
  const semua = await statusSemuaTtd();
