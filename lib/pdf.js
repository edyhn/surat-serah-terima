...
function gambarForm(doc, s) {
  ...
  if (s.aset && s.aset.length) {
    const barisAset = [['Kode', 'Nama Aset', 'Nilai', 'Kondisi']];
    s.aset.forEach((a) => barisAset.push([a.kode, a.nama || '', formatRupiah(a.nilai), a.kondisi || '']));
    y = gambarTabel(doc, L, y, [110, W - 220, 90, 70], barisAset, 5);
    y += 10;
  }
  ...
  const namaTtd = [`(${s.nama})`, `(${s.penerima})`, s.namaHrd ? `(${s.namaHrd})` : '(................)'];
  doc.font(FONT).fontSize(10);
  ...
  if (s.ttd) {
    ...
    labels.forEach((_, i) => {
      const buf = s.ttd[kunciTtd[i]];
      if (buf) {
        const cx = 50 + i * colWidth + colWidth / 2;
        doc.image(buf, cx - 55, yLine - 39, { width: 110, height: 33 });
      }
    });
  }
  namaTtd.forEach((n, i) => {
