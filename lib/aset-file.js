...
function ambilAset(kode) {
  return seri(async () => {
    const arr = bacaJson(FILE_ASET);
    return arr.find((x) => String(x.kode) === String(kode)) || null;
  });
}

function updateAset(kode, a, predicate, _user) {
  return seri(async () => {
    const arr = bacaJson(FILE_ASET);
...
function hapusAset(kode, predicate) {
  return seri(async () => {
    const arr = bacaJson(FILE_ASET);
    const i = arr.findIndex((x) => String(x.kode) === String(kode));
    if (i < 0) return false;
...
function aturStatus(kodeAset, status, _user) {
  return seri(async () => {
    const set = new Set((kodeAset || []).filter((k) => typeof k === 'string').map((k) => k.trim()).filter(Boolean));
...
function tautkanSurat(nomor, kodeAset) {
  return seri(async () => {
    const kodes = [...new Set((kodeAset || []).filter((k) => typeof k === 'string').map((k) => k.trim()).filter(Boolean))];
